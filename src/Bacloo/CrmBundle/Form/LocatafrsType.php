<?php

namespace Bacloo\CrmBundle\Form;

use Symfony\Component\Form\AbstractType;
use Symfony\Component\Form\FormBuilderInterface;
use Symfony\Component\OptionsResolver\OptionsResolverInterface;

class LocatafrsType extends AbstractType
{
    /**
     * @param FormBuilderInterface $builder
     * @param array $options
     */
    public function buildForm(FormBuilderInterface $builder, array $options)
    {
        $builder
            ->add('chantierid', 'integer', array('required' => false))
            ->add('nomchantier', 'text', array('required' => false))
            ->add('adresse1', 'text', array('required' => false))
            ->add('adresse2', 'text', array('required' => false))
            ->add('adresse3', 'text', array('required' => false))
            ->add('cp', 'text', array('required' => false))
            ->add('ville', 'text', array('required' => false))
            ->add('fournisseurid', 'text', array('required' => false))
            ->add('fournisseur', 'text', array('required' => false))
            ->add('user', 'text', array('required' => false))
            ->add('contact', 'text', array('required' => false))
            ->add('datemodif', 'text', array('required' => false))
            ->add('annulee', 'checkbox', array('required' => false))
            ->add('etatbdc', 'checkbox', array('required' => false))
            ->add('montantloc', 'number', array('required' => false))
            ->add('codelocatasl', 'integer', array('required' => false))
            ->add('compteurbdc', 'integer', array('required' => false))
            ->add('mode', 'text', array('required' => false))
            ->add('commentaire','textarea', array('required' => false))
            ->add('locationsfrs', 'collection', array(
			'type'         => new LocationsfrsType(),
			  'allow_add'    => true,
			  'allow_delete' => true,
			  'required'     => true	
		  ))
            ->add('datecrea', 'text', array('required' => false))
            ->add('contributionverte', 'integer', array('required' => false))
            ->add('assurance', 'integer', array('required' => false))
            ->add('sanstva', 'checkbox', array('required' => false))
        ;
    }
    
    /**
     * @param OptionsResolverInterface $resolver
     */
    public function setDefaultOptions(OptionsResolverInterface $resolver)
    {
        $resolver->setDefaults(array(
            'data_class' => 'Bacloo\CrmBundle\Entity\Locatafrs'
        ));
    }

    /**
     * @return string
     */
    public function getName()
    {
        return 'bacloo_crmbundle_locatafrs';
    }
}
