...
// {echo '444';echo $locatafrs->getFournisseur();	
	...
	if($nblocm == $locencours && ($nblocm - $dejafacm) != 0 )
	{
		//Clonage moisder
		echo 'CLONE1';
		$go = 'clonage';
...
echo '11111';
		...
		$newLocata->setcodelocatasl($oldLocata->getCodelocatasl());
...
//								$locations->setFinloc(date('Y-m-t', strtotime(" -1 month")));
						...
						if($loca->getMensuel() == 1)
						{
							...
							if($nbjloc < 20)
							{
								$montantloc = ($loca->getPu()/20)*$nbjloc;echo 'oooooooaaaaaaaaaa'.$nbjloc;
							}
							else
							{
								$montantloc = $loca->getPu();echo 'oooooooxxxx'.$montantloc;
								$nbjloc = 20;
								$nbjlocass = 28;
							}echo 'oooooooxxxx'.$montantloc;
...
echo 'totalht'.$totalht;echo 'transport'.$transportbdc;echo 'assurance'.$assurance;
	...
	//Ajout à la table factures
		...
		if($locatafrs->getMode() == 'recurrent')
        {
            $facture->setEcheance(date('Y-m-t'));
        }
		else
        {
