...
echo ' nblocs'.$nblocs;
					...
					foreach($locataclonedeja2 as $locoa)
					{
						...
						elseif(date("Y-m", strtotime($locoa->getDatemodif())) == date('Y-m', strtotime(" -1 month")))
						{
							$dejafacm1++;
							foreach($locoa->getLocationsclone() as $loca){if($loca->getEtatloc() == 'Location terminée'){$nblocsfact++;}}
							foreach($locoa->getLocationsslclone() as $loca){if($loca->getEtatloc() == 'Location terminée'){$nblocsfact++;}}
						}
					}
echo 'NBLOCFACT'; echo $nblocsfact;echo $nblocs;echo date("Y-m", strtotime($locoa->getDatemodif()));echo date('Y-m', strtotime(" -1 month"));echo $loca->getEtatloc();					
					...
					// elseif($dejafac > 0 && $nblocaclot == $nblocters && $nblocsfact < $nblocs)// en fzait MES24 réuni ces conditions mais n'est pas le seul
...
if(($dejafac == 0 && $nblocters > 0) || ($dejafac == 0 && $touter == 1) || ($nblocaclot != $nblocs && $dejafac == 1 && $nblocters > 0 && $moisder > 0 && $finmois == date('Y-m-t', strtotime(" -1 month") && strtotime($loca->getFinloc()) > $finmoissec))){echo $locatas['f_client'];echo $moisder; echo $moisprec; echo $nblocatersl;echo 'clonage';}else{echo $locatas['f_client']; echo 'pas clonage';}				
					if(($dejafac == 0 && $nblocters > 0) || ($dejafac == 0 && $touter == 1) || ($nblocaclot != $nblocs && $dejafac == 1 && $nblocters > 0 && $moisder > 0 && $finmois == date('Y-m-t', strtotime(" -1 month")) && strtotime($loca->getFinloc()) > $finmoissec && strtotime($loca->getFinloc()) <= strtotime(date('Y-m-d'))))//si au moins une loc démarrée mois dernier et pas de cloone pour ce mois-ci
					{
						echo 'On y va';
...
// echo 'laa';		echo 'nbjloc'.$nbjloc;					
	...
	//On récupère le dernier locationsslclone
									//echo $locata->getId(); echo $loca->getCodemachineinterne();
									$lastlocsl = $em->getRepository('BaclooCrmBundle:Locataclone')
											->lastlocsl($locata->getId(), $loca->getCodemachineinterne());
											// print_r($lastlocsl);
									foreach($lastlocsl as $lasta)
									{//echo $lasta->getNomchantier();
										foreach($lasta->getLocationsslclone() as $last)
										{
											// echo '>>>>>';echo $last->getFinloc();
											$finloc = $last->getFinloc();
											$transportallersl = $last->getTransportaller();
										}
										$lastlocasl = $lasta->getLocationsslclone();
									}
									if(isset($lastlocasl))
									{echo 'lasta';
										$moisderfac = date("M", strtotime($finloc));
									...
									{
										$contributionverte += 4.99;//$contributionverte += $loyer*$nbjloc*0.0215;
									}
									else
									{
										$contributionverte += 0;
