...
// OU
$today = date('Y-m-d'); // Tu peux modifier cette date manuellement si besoin
//$today = '2025-07-01'; // Tu peux modifier cette date manuellement si besoin
$todaysec = strtotime($today); // Timestamp de $today
$premierJourDuMoisEnCours = date('Y-m-01', strtotime($today));
$finDuMoisEnCours = date('Y-m-t', strtotime($today));
$debutMoisPrecedent = date('Y-m-01', strtotime("first day of -1 month", $todaysec));
$finMoisPrecedent = date('Y-m-t', strtotime("last day of -1 month", $todaysec));

$ymMoisPrecedent = date('Y-m', strtotime("last day of -1 month", $todaysec));

if ($mode == 'moisder') {
    // Début et fin du mois précédent basé sur $today
    $debutmoisinit = date('Y-m-01', strtotime("first day of -1 month", $todaysec));
    $debutmois = date('Y-m-01', strtotime("first day of -1 month", $todaysec));
    $finmoisinit = date('Y-m-t', strtotime("last day of -1 month", $todaysec));
} elseif ($mode == 'forcing') {
    // Début et fin du mois courant basé sur $today
    $debutmoisinit = date('Y-m-01', $todaysec);
    $debutmois = date('Y-m-01', $todaysec);
    $finmoisinit = date('Y-m-t', $todaysec);
} else {
    // Début du mois courant, fin au jour de $today
    $debutmoisinit = date('Y-m-01', $todaysec);
    $debutmois = date('Y-m-01', $todaysec);
    $finmoisinit = $today;
}
...
// Mois précédent en abrégé (ex: Jan, Feb...)
$moisprec = date('M', strtotime("first day of -1 month", $todaysec));
...
//                        $debutmois = date('Y-m-01', strtotime(" -1 month"));echo 'debutmoisbaaaaaab'.$debutmois;//Debut mois précédent
//                        $finmoisinit = date('Y-m-t', strtotime(" -1 month"));//Fin du mois précédent
//                    }
//                    elseif($mode == 'forcing')
//                    {//echo 'debutmoisforcing';
...
//                    else
//                    {//echo 'debutmoiselse';
//                        $debutmoisinit = date('Y-m-01');
//                        $debutmois = date('Y-m-01');
//                        $finmoisinit = date('Y-m-d');
//                    }
					$go = 'XXX';echo '<br>****';echo $locatas['f_client'];echo $locatas['f_id'];echo '****';echo 'GOOO11';echo $go;echo 'debutmois'.$debutmois;echo 'mode'.$mode;
					//Partie locations
					...
					foreach($locata->getLocations() as $loca)
					{
                        if($loca->getEtatloc() == 'Location terminée')
                        {
                            $query = $em->createQuery(
                                'SELECT b
                                FROM BaclooCrmBundle:Locationsclone b
                                WHERE b.oldid = :oldid
                                ORDER BY b.id DESC'
                            );
                            $query->setParameter('oldid',$loca->getId());
                            $query->setMaxResults(1);
                            $locationsCloneAControler = $query->getOneOrNullResult();
                            if(!empty($locationsCloneAControler) and strtotime($locationsCloneAControler->getFinloc()) == strtotime($loca->getFinloc()))
                            {
                                echo '<br>iciiiii'.$loca->getId();
                                $loca->setCloture(1);
                                $em->persist($loca);
                                $em->flush();
                            }
                            ...
                            {
                                echo '<br>laaaaaa'.$loca->getId();
//                                echo ' yyyy'.$loca->getFinloc();
                                $loca->setCloture(0);
                                $em->persist($loca);
                                $em->flush();
                            }
...
//						echo ' *finloc';echo $loca->getFinloc();
						if(strtotime($loca->getDebutloc()) <= strtotime($premierJourDuMoisEnCours) and strtotime($loca->getFinloc()) >= $debutmoissecinit){$nblocm1++;}
						
						//m
						if(strtotime($loca->getDebutloc()) <= strtotime($today) and strtotime($loca->getFinloc()) >= $debutmoissecinit){$nblocm++;}
					}
//                    if($locatas['f_id'] == 10006){echo 'tttt';exit();}
					...
					foreach($locata->getLocationssl() as $loca)
					{
                        if($loca->getEtatloc() == 'Location terminée')
                        {
                            $query = $em->createQuery(
                                'SELECT b
                                FROM BaclooCrmBundle:Locationsslclone b
                                WHERE b.oldid = :oldid
                                ORDER BY b.id DESC'
                            );
                            $query->setParameter('oldid', $loca->getId());
...
echo 'GOOO22';echo $go;
					...
					$nblocterm1 = 0;//nb de locs terminées du mois  -1
					$nblocecm = 0;//nb de locs en cours mois m
					$nblocecm1 = 0;//nb de locs en cours mois m-1
					foreach($locata->getLocations() as $loca)
					{
						if(strtotime($loca->getFinloc()) >= strtotime($premierJourDuMoisEnCours) && strtotime(($loca->getFinloc())) <= strtotime($today) && $loca->getEtatloc() != 'En location'){$nblocterm++;}
						if(strtotime($loca->getFinloc()) < strtotime($premierJourDuMoisEnCours) && strtotime(($loca->getFinloc())) >= $debutmoissecinit && $loca->getEtatloc() == 'Location terminée'){$nblocterm1++;}
						if($loca->getEtatloc() == 'En location'){$nblocecm++;}
						if(strtotime($loca->getDebutloc()) < strtotime($premierJourDuMoisEnCours) && strtotime(($loca->getFinloc())) >= $debutmoissecinit){$nblocecm1++;}
					}
					foreach($locata->getLocationssl() as $loca)
					{
						if(strtotime($loca->getFinloc()) >= strtotime($premierJourDuMoisEnCours) && strtotime(($loca->getFinloc())) <= strtotime($today) && $loca->getEtatloc() != 'En location'){$nblocterm++;}
...
echo ' LOCATAS ID '.$locatas['f_id'];
					...
					$nbfactm1 = 0;//nb de facture du mois m-1//nb de facture terminées du mois m-1
					$moisder = 0;
					...
					foreach($locata->getLocations() as $loca)
					{echo '&&locationId'.$loca->getId();
						$debutmoisici = $debutMoisPrecedent;echo 'LOCA DEBUTLOC'.$loca->getDebutloc();echo 'LOCA FINLOC'.$loca->getFinloc();echo ' DEBUT MOISDER '.$debutmoisici;
						//Si loc commencée avant début du mois courant et finit après début mois courant (loc en cours)
						if(strtotime($loca->getDebutloc()) < strtotime($premierJourDuMoisEnCours) && strtotime($loca->getFinloc()) >= strtotime($debutmoisici))
						{
							$moisder++;
							$finmois = $finMoisPrecedent;//Fin mois précédent
							$finmoissec = strtotime ($finmois);
						}
						elseif(strtotime($loca->getDebutloc()) >= strtotime($premierJourDuMoisEnCours) and strtotime($loca->getFinloc()) <= strtotime($today))//au moins une loc commence et finit le mois courant
						{
							// $moisder++;
							$finmois = $today;//Fin mois précédent
							$finmoissec = strtotime ($finmois);
						}
					}
					foreach($locata->getLocationssl() as $loca)
					...
					if(!empty($locataclonedeja22))
					{
						foreach($locataclonedeja22 as $locataclonedeja2)
						{
							...
							{echo ' CODE MACHINE '.$loca->getCodemachineinterne();	echo 'FFIIIIIINNLOC'.date("Y-m", strtotime($loca->getFinloc()));echo 'M-1'.date('Y-m', strtotime(" -1 month"));
								...
								{
									if(date("Y-m", strtotime($loca->getFinloc())) == $ymMoisPrecedent){$dejafacm1++;if($loca->getDef() == 1){$nbfacterm1++;}}
								}

                                if($loca->getEtatloc() == 'Location terminée' && strtotime($loca->getFinloc()) <= strtotime($today))
                                {
                                    //if($locatas['f_id'] == 8805){echo 'idddd'.$loca->getId();exit('stoooopx');}
                                    $locationsAMaj  = $em->getRepository('BaclooCrmBundle:Locations')
                                        ->findOneById($loca->getOldId());
                                    $locationsAMaj->setCloture(1);//echo '$loca->getOldId()'.$loca->getOldId();exit();
                                    $em->persist($locationsAMaj);
                                }
							}
//                            if($locatas['f_id'] == 9668){echo 'idddd'.$loca->getId();exit('stoooopx');}
							...
							{echo ' CODE MACHINESL '.$loca->getCodemachineinterne();echo 'delocym'.date('Ym',strtotime($loca->getDebutloc())); echo 'ym'.date('Ym');
								if(date('Ym',strtotime($loca->getFinloc())) == date('Ym'))
								{
									echo 'BAAXXX';echo 'YM debutloc'.date("Y-m", strtotime($loca->getDebutloc()));echo 'TM DATE'.$ymMoisPrecedent;
									if(date("Y-m", strtotime($loca->getFinloc())) == date("Y-m")){$dejafacm++;if($loca->getDef() == 1){$nbfacterm++;}}
								}
								else
								{echo 'AVANT';
                                ...
                                if($loca->getEtatloc() == 'Location terminée' && strtotime($loca->getFinloc()) <= strtotime($today))
                                {
                                    $locationsSlAMaj = $em->getRepository('BaclooCrmBundle:Locationssl')
                                        ->findOneById($loca->getOldId());
                                    $locationsSlAMaj->setCloture(1);
                                    $em->persist($locationsSlAMaj);
                                }
							}
	echo 'GOOO44';echo $go;
						}
...
echo 'GOOO77';echo $go;
					...
					{echo $mode;echo 'GAGOUYY'.$nblocm;echo $nblocecm;echo $dejafacm; echo $nblocterm;echo '===';echo $nblocm - $nblocecm; echo $dejafacm - $nblocterm;
						...
						{echo 'AAA';
							...
							elseif($mode == 'moisder' && $dejafacm1 > 0 && $dejafacm1 < $moisder)
							{
								//clonage moisder
								echo 'CLONE6';
								$go = 'clonage';
...
echo 'GOOO99';echo $go;
					if($go == 'clonage')
					{
                        ...
                        $client = $em->getRepository('BaclooCrmBundle:Fiche')
                            ->findOneById($oldLocata->getClientid());

						$newLocata->setOldid($oldLocata->getId());
                        $newLocata->setDatemodif($today);
                        $newLocata->setFactor($client->getFactor());
                        $newLocata->setPrixcarb($client->getPrixcarb());
                        $newLocata->setNumbdcloueur($oldLocata->getNumbdcloueur());
						...
						$locata = $em->getRepository('BaclooCrmBundle:Locata')
									->findOneById($locid);
echo 'AAAAAAA';echo $newLocata->getId();echo 'MONTANTLOC'.$newLocata->getMontantloc();
								...
								$clonageloc = 0;
                                $datereprise = 0;
								//On clone si fin de mois et si location terminée même si une loc                                 $loueur = 0;$datereprisen'est pas terminée sur le contrat
								foreach($locata->getLocations() as $loca)
								{echo 'CLONAGE LOCATIONPARC';echo $loca->getCodemachineinterne();
                                    if($loca->getEnergie() == 'diesel' && $loca->getEtatloc() == 'Location terminée')
                                    {
                                        $newLocata->setAlerte('oui');
                                    }
                                    if(null !== $loca->getDatereprise())
                                    {
                                        $datereprise = $loca->getDatereprise();
                                    }
		...
		echo 'transportallerp';$transportallerp;	echo 'moisderfacnum ';echo $moisderfacnum;
									if(($loca->getEtatloc() == 'En location' and strtotime($loca->getDebutloc()) < strtotime($premierJourDuMoisEnCours) and strtotime($loca->getFinloc()) <= strtotime($today) and $moisderfacnum != date('m', strtotime(" -1 month")) and $mode != 'moisder' and mode != 'forcing') or ($loca->getEtatloc() == 'En location' and strtotime($loca->getDebutloc()) < strtotime($premierJourDuMoisEnCours) and $moisderfacnum != date('m', strtotime(" -1 month")) and $mode == 'moisder') or ($loca->getEtatloc() == 'En location' and strtotime($loca->getDebutloc()) <= strtotime($finDuMoisEnCours) and $moisderfacnum != date('m') && $mode == 'forcing') or ($loca->getEtatloc() == 'Location terminée'))
                                    {
										...
										{echo 'cloture1';
											...
											$finmoissec = strtotime ($finmois);
											if(strtotime($loca->getFinloc()) >= strtotime($premierJourDuMoisEnCours) && $mode == 'moisder')
											{}
//											elseif(strtotime($loca->getFinloc()) >= strtotime(date('Y-m-01')) && $loca->getFinloc() <= strtotime(date('Y-m-d')) && $mode != 'moisder')
//											{echo 'cloture1x';
//                                                $loca->setCloture(1);
//                                            }
//											else
...
//											}
										...
										if(($loca->getEtatloc() == 'En location' or ($loca->getEtatloc() == 'Location terminée' && $mode == 'moisder')) && strtotime($loca->getFinloc()) > strtotime($finMoisPrecedent))
										{
											if($mode == 'forcing')
											{
												if(strtotime($loca->getFinloc()) > strtotime($finDuMoisEnCours))
												{
													$finloc = $finDuMoisEnCours;
												}
												else
												{echo 'cloture2';
											...
											{
												$finloc = $finMoisPrecedent;
											}
										}
										elseif(strtotime($loca->getFinloc()) >= strtotime($premierJourDuMoisEnCours) && $mode == 'moisder')
										{
                                        ...
                                        //if($loca->getId() == 8677){echo 'probleme';exit();}
										$em = $this->getDoctrine()->getManager();
										// $query = $em->createQuery(
											// 'SELECT m.oldid
		...
		echo 'FINLOC';echo $finloc;	echo $loca->getCodemachineinterne();echo 'oldid'.$loca->getId();echo 'CLOOOOTURE'.$loca->getCloture();
										$locationsclonedeja = $em->getRepository('BaclooCrmBundle:Locationsclone')
											->findOneBy(array('oldid' => $loca->getId(), 'codemachineinterne' => $loca->getCodemachineinterne(), 'finloc' => $finloc));	if(empty($locationsclonedeja)){echo 'pas locataclonedejaxx';}else{echo 'locataclonedejaxx';}
                                        if((($mode == 'moisder' and strtotime($loca->getDebutloc()) < strtotime($premierJourDuMoisEnCours)) and (($mode == 'moisder' and empty($locationsclonedeja))))
                                           or ($mode != 'moisder' and $loca->getCloture() == 0))
										{
		...
		// echo $dStart; echo $debutmois;
											...
											elseif($dStartsec < strtotime($premierJourDuMoisEnCours) && $dStartsec > $debutmoissec && $moisderfacnum == date('m', strtotime(" -1 month")))//si date début est mois dernier et dernière facturation = mois dernier.
											{echo 'DIDAN2';
												$dStart = $premierJourDuMoisEnCours;
											...
											if($loca->getEtatloc() == 'Location terminée' and $finlocsec <= $todaysec and $finlocsec >= strtotime($premierJourDuMoisEnCours))
											{echo 'CAY';
												$dEnd = $loca->getFinloc();
												if(strtotime($loca->getDebutloc()) < strtotime($premierJourDuMoisEnCours) && $moisderfacnum > 0)
												{echo 'GUY';
											...
											if($finlocsec >= $finmoissec)
											{
												...
												{echo 'LAAAA';echo $finmois;
													if($loca->getEtatloc() == 'En location')
													{echo 'ico';
														if($mode == 'forcing' && $finlocsec >= strtotime( $finDuMoisEnCours))
														{
															$finmois = $finDuMoisEnCours;//Fin mois en cours
														}
														elseif($mode == 'moisder')
														{
															$finmois = $finMoisPrecedent;//Fin mois en cours
														}
														else
														{echo 'cloture3';
                                                            if(strtotime($loca->getFinloc()) > strtotime($finDuMoisEnCours))
		...
		echo $end->format('Y-m-d');
											...
											foreach ($period as $dt) {
												$time = strtotime($dt->format("Y-m-d"));//echo $dt->format("Y-m-d");
                                                $newformat = date('D',strtotime($dt->format("Y-m-d")));
                                                // echo $dat;
...
//												$nbjloc = 1;
		...
		// echo 'laa';		echo 'nbjloc'.$nbjloc;
                                            ...
                                            $locations->setRemise($loca->getRemise());
											...
											else
											{
                                                ...
                                                elseif($loca->getLoyerp2() > 0)
                                                {
                                                    $loyer = $loca->getLoyerp2();
                                                }
                                                elseif($loca->getLoyerp3() > 0)
                                                {
                                                    $loyer = $loca->getLoyerp3();
                                                }
                                                elseif($loca->getLoyerp4() > 0)
                                                {
                                                    $loyer = $loca->getLoyerp4();
                                                }
											}
											if($locations->getRemise() > 0)
                                            {
                                                $remise = (($nbjloc * $loyer) - $jour50 - $jour100) * $locations->getRemise()/100;
                                                $locations->setMontantloc(($nbjloc * $loyer) - $jour50 - $jour100 - $remise);
                                            }
											else
                                            {
											...
											elseif($loca->getEtatloc() == 'Location terminée' && strtotime($loca->getDebutloc()) < $debutmoissecinit && strtotime($loca->getFinloc()) >= strtotime($premierJourDuMoisEnCours))
											{echo 'START';echo $moisderfac;echo $moisprec;
												...
												{echo 'pas moisder2';
													$locations->setDebutloc($premierJourDuMoisEnCours);
												}
											}
											elseif($loca->getEtatloc() == 'Location terminée' && strtotime($loca->getDebutloc()) < $debutmoissecinit && strtotime($loca->getFinloc()) < strtotime($premierJourDuMoisEnCours))
											{echo 'STARTX';echo $moisderfac;echo $moisprec;
												if($mode == 'moisder')
												{echo 'moisder2';
													$locations->setDebutloc($debutmoisinit);
												}
											...
											elseif($loca->getEtatloc() == 'Location terminée' && strtotime($loca->getDebutloc()) >= $debutmoissecinit && strtotime($loca->getFinloc()) >= strtotime($premierJourDuMoisEnCours) && $moisderfac === $moisprec)
											...
											if(($loca->getEtatloc() == 'En location' or ($loca->getEtatloc() == 'Location terminée' && $mode == 'moisder')) && strtotime($loca->getFinloc()) > strtotime($finMoisPrecedent))
											{
												if($mode == 'forcing')
												{
													if(strtotime($loca->getFinloc()) > strtotime($finDuMoisEnCours))
													{
														$locations->setFinloc($finDuMoisEnCours);
														$finlocsel = $finDuMoisEnCours;
													}
													else
													{
												...
												elseif($mode == 'moisder')
												{echo '&&&&&&&&&';
													$locations->setFinloc($finMoisPrecedent);
													$finlocsel = $finMoisPrecedent;
												}
												else
												{echo 'AAAAAAAAAAAA';
                                                    if(strtotime($loca->getFinloc()) > strtotime($finDuMoisEnCours))
...
echo 'debutmois'.$debutmois;echo 'finmois'.$finmoisinit;
                                            ...
                                            if($loca->getContributionverte() == 1)
                                            {
                                                if($locations->getRemise() > 0)
                                                {
                                                    $remise = $nbjloc * $loyer * $locations->getRemise()/100;
                                                    $contributionverte += (($nbjloc  * $loyer) - $remise)*0.0215;
                                                }
                                                else
                                                {
                                                    $contributionverte += $loyer*($nbjloc - $jour50 - $jour100)*0.0215;
                                                }
                                            }
                                            else
                                            {
                                            ...
                                            if($loca->getAssurance() == 1)
                                            {
                                                if($locations->getRemise() > 0)
                                                {
                                                    $remise = ($nbjlocass * $loyer * 0.10) * $locations->getRemise()/100;
                                                    $assurance += ($loyer*$nbjlocass*0.10)-$remise;
                                                }
                                                else
                                                {
                                            ...
                                            if(null !== $loca->getLoyerp1())
                                            {
                                                ...
                                                $loyer = $loca->getLoyerp1();
                                                if ($loca->getRemise() > 0) {
                                                    $loyer = $loyer - ($loyer * ($loca->getRemise() / 100));
                                                }
...
//											echo 'locationsclonedeja pas vide';echo $locationsclonedeja->getId();
                                ...
                                $loueurid = 0;
								foreach($locata->getLocationssl() as $loca)
								{echo 'CLONAGE LOCATION';echo $loca->getCodemachineinterne();echo 'locaid'.$loca->getId();
                                    if($loca->getEnergie() == 'diesel' && $loca->getEtatloc() == 'Location terminée')
                                    {
                                        $newLocata->setAlerte('oui');
                                    }
                                    if(!empty($loca->getLoueur()))
                                    {
                                        $loueur = $loca->getLoueur();
                                        $loueurid = $loca->getLoueurid();
                                    }
                                    if(!empty($loca->getDatereprise()))
                                    {
                                        $datereprise = $loca->getDatereprise();
                                    }
									//On récupère le dernier locationsslclone
		...
		// echo 'xxxxxxxxxxxxxxxxx';echo $finmois; echo $loca->getFinloc();
											...
											{echo 'CAY2';
												...
												{echo 'GUY2';
                                                    ...
                                                    if($mode == 'moisder' && strtotime($loca->getDebutloc()) > strtotime($debutMoisPrecedent))//si moisder et debutloc est sur le mois dernier
                                                    {echo 'moisderDstart';
                                                        $dStart = $loca->getDebutloc();
                                                    }
                                                    elseif($mode == 'moisder' && strtotime($loca->getDebutloc()) > strtotime($debutMoisPrecedent))//si moisder et debutloc pas sur le mois dernier
                                                    {echo 'moisderDstart';
                                                        $dStart = $debutmoisinit;
                                                    }
                                                    else
											...
											if($finlocsec >= $finmoissec)
											{
												...
												{echo 'cloture6';
                                                    if(strtotime($loca->getFinloc()) >= strtotime($premierJourDuMoisEnCours) && $mode == 'moisder')
                                                    {
                                                        $dEnd = $finMoisPrecedent;
                                                        $finmois = $finMoisPrecedent;
                                                        $finmoissec = strtotime ($finmois);
                                                    }
                                                    else
                                                    {
                                                        $dEnd = $loca->getFinloc();echo 'dendxxxxxx'.$dEnd;
                                                        $finmois = $loca->getFinloc();
                                                        $finmoissec = strtotime ($finmois);
                                                        $loca->setCloture(1);
                                                    }
												...
												else
												{
													if($loca->getEtatloc() == 'En location')
													{
														if($mode == 'forcing')
														{
															$finmois = $finDuMoisEnCours;//Fin mois précédent
															$finmoissec = strtotime ($finmois);
														}
														else
														{
...
echo ' MOOOODE'.$mode; echo 'finloc'.$finloc;  echo ' debutmois'.$debutmoisinit; echo ' debutloc'.$loca->getDebutloc(); echo ' finmois'.$finmoissec;
											...
											{echo 'DTART FORCING';
												...
												if(strtotime($loca->getDebutloc())< strtotime($premierJourDuMoisEnCours))
												{echo 'debut111';
													$dStart = $premierJourDuMoisEnCours;
												}
												elseif(strtotime($loca->getDebutloc()) >= strtotime($premierJourDuMoisEnCours) && strtotime($loca->getDebutloc()) <= strtotime($finDuMoisEnCours))
												{echo 'debut222';
													$dStart = $loca->getDebutloc();
												}
												//Détermination dEnd
												if(strtotime($loca->getFinloc()) > strtotime($finDuMoisEnCours))
												{echo 'debut333';
													$dEnd = $finDuMoisEnCours;
													$finmois = $finDuMoisEnCours;
													$finmoissec = strtotime ($finmois);
												}
												elseif(strtotime($loca->getFinloc()) >= strtotime($premierJourDuMoisEnCours) && strtotime($loca->getFinloc()) <= strtotime($finDuMoisEnCours))
												{
													$dEnd = $loca->getFinloc();
													$finmois = $loca->getFinloc();
													$finmoissec = strtotime ($finmois);
											...
											//Si date fin posterieure à fin du mois >> date début = début mois
												...
												$feries = $this->getHolidays(date('Y'));
												foreach ($period as $dt) {
													$time = strtotime($dt->format("Y-m-d"));//echo '<br>1158'.$dt->format("Y-m-d");echo '<br>time'.date('m-d', $time);
                                                    $newformat = date('D',strtotime($dt->format("Y-m-d")));
													// echo $dat;
													//Calcul nb jours fériés
													$i = 0;
											...
											$locations->setCodeclient($loca->getCodeclient());
											$locations->setLoueur($loca->getLoueur());
											$locations->setLoueurid($loca->getLoueurid());
											$locations->setEntid($loca->getCodeclient());
											$locations->setEntreprise($loca->getEntreprise());
											$locations->setMachineid($loca->getMachineid());
											$locations->setCodemachine($loca->getCodemachine());
                                            ...
                                            if($locations->getRemise() > 0)
                                            {
                                                $remise = ($nbjloc - $jour50 - $jour100) * $loyer * $locations->getRemise()/100;
                                                $locations->setMontantloc((($nbjloc - $jour50 - $jour100 ) * $loyer) - $remise);
                                            }
                                            else
                                            {
                                                $locations->setMontantloc(($nbjloc- $jour50 - $jour100) * $loyer) ;
                                            }
...
echo ' YM01 ';echo $premierJourDuMoisEnCours;
...
//                        if($premiermois > 0)
                            if(null !== $locata->getLocataventes())
                            {//echo 'premier mois locavente';
                                ...
                                if(!empty($newLocata->getCommentairecache()))
                                {
                                    $newLocata->setAlerte('oui');
                                }
								$em->persist($newLocata);
								...
								else
								{
									...
									//Fin ajout table factures
										...
										//Ajout à la table factures
											...
											if($loueur != '0')
                                            {
                                                $facture->setLoueur($loueur);
                                                $facture->setLoueurid($loueurid);
                                            }
											if($datereprise != 0)
                                            {
                                                $facture->setDatereprise($datereprise);
                                            }
											...
											$facture->setPrixcarb($newLocata->getPrixcarb());
                                            if($mode == 'forcing')
                                            {
                                                $facture->setDatecrea($finDuMoisEnCours);
                                                $datecrea = $finDuMoisEnCours;
                                            }
                                            elseif($mode == 'moisder')
                                            {
                                                $facture->setDatecrea($finMoisPrecedent);
                                                $datecrea = $finMoisPrecedent;
                                            }
                                            else
                                            {
                                            ...
                                            {
                                                $date = date('Y-m-d', strtotime($datecrea . '+45 days'));
                                                $next45 = date('Y-m-t', strtotime($date));
                                            }
                                            elseif($client->getDelaireglement() == 3)
                                            {
                                                $date = date('Y-m-d', strtotime($datecrea . '+30 days'));
                                                $next45 = date('Y-m-t', strtotime($date));
                                            }
                                            else
                                            {
                        ...
                        $facture->setMois(date('m', strtotime($datecrea)));
											...
											$em->persist($client);
											$em->flush();
                                            $newLocata->setDatemodif($facture->getDatecrea());
                                            $em->persist($newLocata);
                                            $em->flush();
											//Fin ajout table factures
//                        echo 'BBBBB'.$newLocata->getMontantloc().$newLocata->getClient().$newLocata->getNomchantier();echo 'idyyyy'.$id;

                        if($newLocata->getMontantloc() == 0 and $clonageloc == 0)
                        {//echo 'yesss';exit();
                            $em->remove($newLocata);
                            $em->remove($facture);
                        }
