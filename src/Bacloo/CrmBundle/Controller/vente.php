...
$facture  = $em->getRepository('BaclooCrmBundle:Factures')
    ->findOneBy(array('locatacloneid'=> $venda->getId(), 'typedoc'=> 'facture', 'client'=> $client->getRaisonSociale()));
if(isset($facture) and $facture->getEncompta() == 1)
{
    return $this->redirect($this->generateUrl('bacloocrm_compta', array('vue' => 'locations', 'page' => 1)));
}
$form = $this->createForm(new VendaType(), $venda);
$request = $this->get('request');//echo '<br>avant poste'.$request->getMethod();
...
//Ajout à la table factures
                    ...
                    {
                        $totalhtfac = $venda->getMontantvente() - $venda->getRemise();
                    }
                    else
                    {
