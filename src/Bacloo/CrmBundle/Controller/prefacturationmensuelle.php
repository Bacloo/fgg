...
echo 'GOOO22';echo $go;
					...
					foreach($locata->getLocations() as $loca)
					{
						if(strtotime($loca->getFinloc()) >= strtotime(date('Y-m-01')) && strtotime(($loca->getFinloc())) <= strtotime(date('Y-m-d')) && $loca->getEtatloc() != 'En location'){$nblocterm++;}
					...
					if(!empty($locataclonedeja22))
					{
						foreach($locataclonedeja22 as $locataclonedeja2)
						{
							...
							{echo ' CODE MACHINE '.$loca->getCodemachineinterne();	echo 'FFIIIIIINNLOC'.date("Y-m", strtotime($loca->getFinloc()));echo 'M-1'.date('Y-m', strtotime(" -1 month"));
								...
								{
									if(date("Y-m", strtotime($loca->getFinloc())) == date('Y-m', strtotime("first day of -1 month"))){$dejafacm1++;if($loca->getDef() == 1){$nbfacterm1++;}}
								}

                                if($loca->getEtatloc() == 'Location terminée' && strtotime($loca->getFinloc()) <= strtotime(date('Y-m-d')))
                                {
...
//                            if($locatas['f_id'] == 9668){echo 'idddd'.$loca->getId();exit('stoooopx');}
							...
							{echo ' CODE MACHINESL '.$loca->getCodemachineinterne();echo 'delocym'.date('Ym',strtotime($loca->getDebutloc())); echo 'ym'.date('Ym');
								if(date('Ym',strtotime($loca->getFinloc())) == date('Ym'))
								{
									echo 'BAAXXX';echo 'YM debutloc'.date("Y-m", strtotime($loca->getDebutloc()));echo 'TM DATE'.date('Y-m', strtotime("first day of -1 month"));
									if(date("Y-m", strtotime($loca->getFinloc())) == date("Y-m")){$dejafacm++;if($loca->getDef() == 1){$nbfacterm++;}}
								}
								else
								{echo 'AVANT';
...
//											}
		...
		echo 'FINLOC';echo $finloc;	echo $loca->getCodemachineinterne();echo 'oldid'.$loca->getId();echo 'CLOOOOTURE'.$loca->getCloture();
                                        ...
                                        if((($mode == 'moisder' and strtotime($loca->getDebutloc()) < strtotime(date('Y-m-01'))) and (($mode == 'moisder' and empty($locationsclonedeja))))
		...
		// echo 'xxxxxxxxxxxxxxxxx';echo $finmois; echo $loca->getFinloc();
											...
											{echo 'CAY2';
												...
												{echo 'GUY2';
                                                    ...
                                                    if($mode == 'moisder' && strtotime($loca->getDebutloc()) > strtotime(date('Y-m-01', strtotime(" -1 month"))))//si moisder et debutloc est sur le mois dernier
                                                    {echo 'moisderDstart';
                                                        $dStart = $loca->getDebutloc();
                                                    }
                                                    elseif($mode == 'moisder' && strtotime($loca->getDebutloc()) > strtotime(date('Y-m-01', strtotime(" -1 month"))))//si moisder et debutloc pas sur le mois dernier
                                                    {echo 'moisderDstart';
                                                        $dStart = $debutmoisinit;
                                                    }
                                                    else
								...
								else
								{
									...
									//Fin ajout table factures
										...
										//Ajout à la table factures
                                            ...
                                            elseif($mode == 'moisder')
                                            {
                                                $facture->setDatecrea(date('Y-m-t', strtotime(" -1 month")));
                                                $datecrea = date('Y-m-t', strtotime(" -1 month"));
                                            }
                                            else
                                            {
