...
class LocataRepository extends \Doctrine\ORM\EntityRepository
{
    ...
    public function devisfaits($du, $au, $user, $nbparpage, $page)
    {
        $qb = $this->createQueryBuilder('f')
            ->leftJoin('f.locations', 'c')
            ->leftJoin('f.locationssl', 's')
    ...
    public function locsmachine($codemachineinterne)
    {        $qb = $this->createQueryBuilder('f')
        ->leftJoin('f.locations', 'c')
        ->addSelect('c');
        $qb->Where('f.contrat = :contrat')
            ->setParameter('contrat', 1)
            ->andWhere('c.codemachineinterne = :codemachineinterne')
            ->setParameter('codemachineinterne', $codemachineinterne);

        return $qb->getQuery()->getResult();
    }

    public function locplanningparc()
    {
        $debutloc = date('Y-m-01', strtotime('-1 month'));
        $finloc = date('Y-m-t', strtotime('+3 month'));
        $qb = $this->createQueryBuilder('f')
            ->leftJoin('f.locations', 'c')
            ->addSelect('c');
//        $qb->where('f.contrat = :contrat')
//            ->setParameter('contrat', 1);
        $qb->where('c.finloc >= :debutloc and c.debutloc <= :finloc')
            ->setParameter('debutloc', $debutloc)
            ->setParameter('finloc', $finloc);
        $qb->orderBy('c.debutloc', 'ASC');

        return $qb->getQuery()->getResult();
    }

    public function locplanningsl()
    {
        $today = date('Y-m-d');
        $debutloc = date('Y-m-01', strtotime('-1 month'));
        ...
        $qb->where('f.contrat = :contrat')
            ->setParameter('contrat', 1);
        $qb->andwhere('c.finloc >= :debutloc and c.debutloc <= :finloc')
            ->setParameter('debutloc', $debutloc)
            ->setParameter('finloc', $finloc);
        $qb->andwhere('c.etatloc != :etatloc ')
            ->setParameter('etatloc', 'Location terminée');
        $qb->orderBy('c.debutloc', 'ASC');
