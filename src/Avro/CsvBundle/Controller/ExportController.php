...
//             $debutmois = '2022-09-01';//En cas de choix de la date
            $finmois = date('Y-m-d');//Fin mois précédent
//             $finmois = '2022-09-30';//Date du jour
        ...
        elseif($alias == 'listedevis')
        {
            $qb->select('o.datecrea, o.entite, o.suivipar, o.client, o.cp, o.ville');
            $qb->Where('o.id > :entite')
                ->setParameter('entite', 0);
            if($dstart != '0' and $dstart != '')
            {//echo 'dduuu'.$du;
                $qb->andWhere('o.datecrea >= :datecrea')
                    ->setParameter('datecrea', $dstart);
            }
            if($dend != '0' and $dend != '')
            {//echo 'auuu'.$au;
                $qb->andWhere('o.datecrea <= :datecrea2')
                    ->setParameter('datecrea2', $dend);
            }
            if($user != '0' and $user != '')
            {//echo 'auuu'.$au;
